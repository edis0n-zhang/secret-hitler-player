...
def generate_startup_prompt(role, db=None):
    prompt = f'''
    You are playing the board game “Secret Hitler.” You will be playing a game with 7 total players.

    It is important to note that this is a game about deception and strategy, your goal is to convince the other players that you are on their side, while secretly working to enact your own agenda.

    The only true source of truth is the game log and current game state you know. You should not fully trust the other players, as they may be lying to you.
    ...
    If you are Hitler, then you are also a Fascist.

    You must wait your turn before committing an action.
    ...
    You will be prompted when it is your turn. For reference, here are the rules of the game: {db["rules"] if db else None}
    '''
    return prompt
